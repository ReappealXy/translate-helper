...
<<<<<<< HEAD
  ...
  async performTranslation(text, resultElement) {
    ...
    try {
      ...
      if (data.responseData && data.responseData.translatedText) {
        resultElement.innerHTML = data.responseData.translatedText;
      } else {
  ...
  async translateLongText(text, resultElement) {
    ...
    resultElement.innerHTML = '<div class="translator-loading">长文本分段翻译中...</div>';
    ...
    for (let i = 0; i < segments.length; i++) {
      ...
      try {
        ...
        if (data.responseData && data.responseData.translatedText) {
          ...
          // 更新进度
          resultElement.innerHTML = `<div class="translator-loading">翻译中...${Math.round((i + 1) / segments.length * 100)}%</div>`;
        } else {
    ...
    if (translationFailed) {
      resultElement.innerHTML = '<div class="translator-error">部分文本翻译失败，请尝试减少文本长度或稍后重试</div>';
    } else {
      resultElement.innerHTML = translatedText.trim();
    }
  }
    ...
    for (const paragraph of paragraphs) {
      if (currentSegment.length + paragraph.length + 1 <= maxLength) {
        ...
        if (currentSegment) {
          ...
          const sentenceSegments = this.splitBySentence(paragraph, maxLength);
      ...
      if (currentSegment.length + sentence.length <= maxLength) {
        ...
        if (currentSegment) {
          ...
          while (remaining.length > 0) {
            segments.push(remaining.substring(0, maxLength));
